...
def make_months_options(months_back: int = 24, months_ahead: int = 24):
    today = date.today()
    y0 = today.year
    m0 = today.month
    out = set()
...
def applicable_budgets_by_category(db, user_id: int, month: str):
    ...
    Preference:
      - One-time where Budget.month == month wins over recurring
      - Else recurring
    """
    rows = db.execute(
    ...
    for b, c in rows:
        ...
        is_one_time_this_month = (b.recurrence != "monthly" and b_month_norm == month)
...
def dashboard():
    ...
    months_options = make_months_options()
    ...
    spent_map = {cid: Decimal(str(total)) for cid, total in spent_rows}

    cards = []
    for cid, (b, c) in sorted(by_cat.items(), key=lambda kv: kv[1][1].name.lower()):
        spent = spent_map.get(cid, Decimal("0"))
        pct = float((spent / b.amount) * 100) if b.amount and b.amount > 0 else 0.0
        pct = min(pct, 999.0)
        cards.append(
            {
                "category_id": cid,
    ...
    unbudgeted_spent_total = Decimal("0.00")
    for cid, total in spent_rows:
        if cid not in budgeted_cat_ids:
            unbudgeted_spent_total += Decimal(str(total))
    unbudgeted_spent = f"{unbudgeted_spent_total:.2f}"
...
def budgets():
    ...
    if request.method == "POST" and "month" in request.form:
        form.month.data = normalize_month(request.form.get("month"))

    if form.validate_on_submit():
        m_norm = normalize_month(form.month.data)
        existing = db.execute(
            select(Budget).where(
                and_(
...
def budgets_edit(budget_id: int):
    ...
    if request.method == "GET":
        ...
        form.month.data = normalize_month(budget.month)
        form.amount.data = budget.amount
        form.recurrence.data = budget.recurrence or "one_time"
    ...
    if form.validate_on_submit():
        budget.category_id = form.category_id.data
        budget.month = normalize_month(form.month.data)
        budget.amount = form.amount.data
        budget.recurrence = form.recurrence.data or "one_time"
        db.commit()
